...
namespace Modbus.IO
{
    ...
    public abstract class ModbusTransport : IDisposable
    {
                ...
                try
                {
                    lock (_syncLock)
                    {
                        ...
                        do
                        {
                            ...
                            else if (!ValidateResponse(message, response))
                            {
                                readAgain = true;
                            }
                        } while (readAgain);
        ...
        /// <summary>
        /// Ensures that the response matches the request.  Allows the transport to re-read a response if necessary, 
        /// but throws an exception if the problem is too serious.
        /// </summary>
        /// <param name="request">Request that was sent</param>
        /// <param name="response">Response to verify </param>
        /// <returns>True if the request matches the response, False if the transport should attempt to re-read the
        /// response. </returns>
        internal bool ValidateResponse(IModbusMessage request, IModbusMessage response)
        {
            ...
            return OnValidateResponse(request, response);
        }

        /// <summary>
        /// Provide hook to do transport level message validation.  Throws exception if a non-recoverable problem 
        /// occured with the response
        /// </summary>
        /// <param name="request">Request that was sent</param>
        /// <param name="response">Response to validate against the request</param>
        /// <returns>True if the response matches the request, False if a re-read should take place</returns>
        internal abstract bool OnValidateResponse(IModbusMessage request, IModbusMessage response);
